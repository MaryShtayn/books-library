...
function createPreviewMarkup({ id, title, author, img, plot }) {
  const markupPreview = `<div data-id = '${id}' class = "preview-box">
         ...
         <p class = "preview-text">${plot}</p></div>`;
...
function deleteBook(e) {
  ...
  setTimeout(() => alert('The book was successfully deleted'), 300);
}
...
function addBook() {
  ...
  function onFormSubmit(e) {
    e.preventDefault();
    const valueNewBook = Object.values(newBook);

    if (valueNewBook.some(value => value === '')) {
...
function editBook(evt) {
  ...
  function onFormSubmit(evt) {
    ...
    setTimeout(() => alert('The book was successfully editted'), 500);
  }
}
